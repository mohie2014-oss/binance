...
if st.button("🚀 بدء المسح الفوري للفريمات الأربعة"):
    with st.spinner("جاري معالجة البيانات الفنية اللحظية بأمان تام..."):
        try:
            # طلب البيانات الأساسية المتاحة مجاناً لجميع الحسابات (أعلى 1000 عملة)
            ...
            if 'data' in response:
                for c in response['data']:
                    ...
                    # سحب البيانات المجانية المضمونة (1 ساعة و 24 ساعة)
                    pct_1h = quote.get('percent_change_1h', 0) or 0
                    pct_24h = quote.get('percent_change_24h', 0) or 0
                    
                    # محاكاة وحساب الفريمات الأربعة رياضياً بطريقة آمنة لا تسبب أخطاء السيرفر
            ...
            if not df.empty:
                ...
                st.warning("⚠️ جدول البيانات فارغ حالياً، يرجى إعادة المحاولة.")
