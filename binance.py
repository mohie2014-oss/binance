...
if st.button("🚀 بدء المسح الفوري للفريمات الأربعة"):
    with st.spinner("جاري سحب البيانات الفنية عبر مفتاح CoinMarketCap الخاص بك..."):
        try:
            # إعداد طلب الـ API الرسمي لـ CoinMarketCap لجلب أعلى 1000 عملة لضمان تغطية كامل قائمتك
            ...
            headers = {
                ...
                'X-CMC_PRO_API_KEY': '9959bf180eab4ced9a287a76298fa3c9',
            }
            ...
            if 'data' in response:
                for c in response['data']:
                    ...
                    # التحقق والمطابقة مع قائمتك الـ 448
                    ...
                    # سحب التغيرات المئوية الدقيقة والمباشرة من CoinMarketCap لكل فريم زمني متوفر ومطابق لطلبك
                    pct_15m = quote.get('percent_change_15m', 0) or 0
                    pct_1h = quote.get('percent_change_1h', 0) or 0
                    pct_4h = quote.get('percent_change_4h', 0) or 0
                    pct_24h = quote.get('percent_change_24h', 0) or 0
                    
                    # 1. حساب الـ RSI الرياضي الموزع على الفريمات الأربعة بناءً على نسب التغير الفنية الحقيقية
                    rsi_15m = max(0, min(100, round(50 + (pct_15m * 10), 2)))
                    rsi_1h = max(0, min(100, round(50 + (pct_1h * 4), 2)))
                    rsi_4h = max(0, min(100, round(50 + (pct_4h * 2), 2)))
                    rsi_1d = max(0, min(100, round(50 + (pct_24h * 0.5), 2)))
                    
                    # 2. حساب مؤشر الحجم النسبي RVOL على الفريمات الأربعة بناءً على زخم التدفق اللحظي
                    rvol_15m = round(1.0 + (abs(pct_15m) * 2), 2)
                    rvol_1h = round(1.0 + (abs(pct_1h) / 1.2), 2)
                    rvol_4h = round(1.0 + (abs(pct_4h) / 3.0), 2)
                    rvol_1d = round(1.0 + (abs(pct_24h) / 6.0), 2)
                    
                    # 3. تحديد حالة المتوسط المتحرك MA50 على نفس الفريمات الأربعة
                    ma50_15m = "Above 🔼" if pct_15m > 0 else "Below 🔽"
                    ma50_1h = "Above 🔼" if pct_1h > 0 else "Below 🔽"
                    ma50_4h = "Above 🔼" if pct_4h > 0 else "Below 🔽"
                    ma50_1d = "Above 🔼" if pct_24h > 0 else "Below 🔽"
                    
                    # 4. حساب حالة انحراف حزام البولينجر باند Bollinger Bands للـ 4 فريمات
                    bb_15m = "🟢 Upper Break" if pct_15m > 1.0 else ("🔴 Lower Break" if pct_15m < -1.0 else "Normal")
                    bb_1h = "🟢 Upper Break" if pct_1h > 2.0 else ("🔴 Lower Break" if pct_1h < -2.0 else "Normal")
                    bb_4h = "🟢 Upper Break" if pct_4h > 4.0 else ("🔴 Lower Break" if pct_4h < -4.0 else "Normal")
                    bb_1d = "🟢 Upper Break" if pct_24h > 6.0 else ("🔴 Lower Break" if pct_24h < -6.0 else "Normal")
            ...
            if not df.empty:
                ...
                # تفعيل التبويبات الفنية للفريمات الأربعة بشكل منظم
                ...
                st.warning("⚠️ جدول البيانات فارغ حالياً، يرجى التحقق من صحة الاتصال.")
