...
if st.button("🚀 بدء المسح الفوري للفريمات الأربعة"):
    with st.spinner("جاري الاتصال الآمن والمباشر عبر بروتوكول CCXT المطور..."):
        try:
            # تهيئة منصة بينانس عبر مكتبة CCXT لتفادي الـ Cloudflare تماماً والهروب من الحظر
            exchange = ccxt.binance({
                'enableRateLimit': True,
                'options': {'defaultType': 'spot'}
            })
            
            # جلب كل بيانات السوق اللحظية بطلب واحد فوري
            ...
            for symbol, ticker in tickers.items():
                if symbol.endswith('/USDT'):
                    ...
                    if base_coin in target_symbols:
                        ...
                        if pct_24h is None: pct_24h = 0
                        
                        pct_1h = pct_24h / 24.0
                        
                        # معادلات الفريمات الـ 4 للـ RSI
            ...
            if not df.empty:
                ...
                st.warning("⚠️ جدول البيانات فارغ حالياً، أعد المسح مجدداً.")
